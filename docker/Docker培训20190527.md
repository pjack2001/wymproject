...
navicat premium乱码解决
2018年12月14日 12:13:38 x_xuyuan 阅读数：339
1.将安装目录下的./start_navicat中的字符集改为zh_CN.UTF-8
2.在工具选项中将常规、编辑器、记录的字体都改为Noto Sans mono CJK SC Regular
3.重启

注意：因为显示乱码，直接复制“Noto Sans mono CJK SC Regular”，工具->选项->字体->界面字体等所有字体都粘贴即可


试用过期，先把链接、试图拷贝出来，删除~/.navicat64，重新打开navicat即可，退出，再把链接拷贝回去

# w @ uw in ~/.navicat64/Navicat [13:10:46] 

++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

在ubuntu下破解Navicat Premium，无限次使用，保留原有的视图sql

破解方案：

第一次执行start_navicat时，会在用户主文件夹下生成一个名为.navicat的隐藏文件夹，该文件夹下的Navicat/Mysql就是所有数据库的视图sql。

第一步：先备份.navicat目录
mv .navicat .navicat_bak

第二步：运行navicat，会自动生成.navicat目录，这时已经再次试用30天了
./start_navicat
